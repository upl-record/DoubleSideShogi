...
bool Position::is_immediate_game_end(Value& result, int ply) const {
  ...
  // Board-only non-royal extinction.  This is useful for drop variants where
  // pieces in hand should not prevent a terminal material-loss rule.
  if (extinction_value() != VALUE_NONE && var->extinctionNonRoyalBoard)
  {
      for (Color c : { sideToMove, ~sideToMove })
          if (!(pieces(c) & ~(pieces(c, KING) | pieces(c, COMMONER))))
          {
              result = c == sideToMove ? extinction_value(ply) : -extinction_value(ply);
              return true;
          }
