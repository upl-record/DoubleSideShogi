...
namespace ffish {
  ...
  std::string json_escape(std::string text) {
    std::string escaped;
    for (char ch : text) {
  ...
  void ensure_search_ready(const Variant* v) {
    if (!Board::sfInitialized) {
      initialize_stockfish();
      Board::sfInitialized = true;
    }
  ...
  std::string analyze(std::string uciVariant, std::string fen, int depth, int multipv, int movetimeMs) {
    ...
    try {
      ...
      if (safeMovetimeMs > 0)
        limits.movetime = safeMovetimeMs;
      else
      ...
      const TimePoint elapsed = std::max<TimePoint>(1, now() - limits.startTime);
      const Search::RootMoves& rootMoves = Threads.main()->rootMoves;
      const int resolvedDepth = Threads.main()->completedDepth;
      const uint64_t nodes = Threads.nodes_searched();

      std::ostringstream json;
      json << "{";
      json << "\"ok\":true";
      json << ",\"engine\":\"ffish-wasm\"";
      json << ",\"variant\":\"" << json_escape(uciVariant) << "\"";
      json << ",\"fen\":\"" << json_escape(fen) << "\"";
      json << ",\"depth\":" << resolvedDepth;
      json << ",\"requestedDepth\":" << safeDepth;
      json << ",\"movetimeMs\":" << safeMovetimeMs;
      json << ",\"multipv\":" << safeMultiPv;
      json << ",\"nodes\":" << nodes;
      json << ",\"nps\":" << (nodes * 1000 / elapsed);
      json << ",\"time\":" << elapsed;
      json << ",\"bestmove\":";
      if (!rootMoves.empty() && !rootMoves[0].pv.empty() && rootMoves[0].pv[0] != MOVE_NONE)
        json << "\"" << json_escape(UCI::move(pos, rootMoves[0].pv[0])) << "\"";
      else
        json << "null";
      json << ",\"lines\":[";

      size_t emitted = 0;
      const size_t lineLimit = std::min(rootMoves.size(), static_cast<size_t>(safeMultiPv));
      for (size_t index = 0; index < lineLimit; ++index) {
        const Search::RootMove& rootMove = rootMoves[index];
        if (rootMove.pv.empty() || rootMove.pv[0] == MOVE_NONE || rootMove.score == -VALUE_INFINITE)
          continue;

        std::string scoreText = UCI::value(rootMove.score);
        std::istringstream scoreStream(scoreText);
        std::string scoreType = "cp";
        int score = 0;
        scoreStream >> scoreType >> score;
        ...
        json << "{";
        json << "\"multipv\":" << emitted;
        json << ",\"depth\":" << resolvedDepth;
        json << ",\"score\":" << score;
        json << ",\"scoreType\":\"" << json_escape(scoreType) << "\"";
        json << ",\"nodes\":" << nodes;
        json << ",\"nps\":" << (nodes * 1000 / elapsed);
        json << ",\"time\":" << elapsed;
        json << ",\"pv\":[";
        for (size_t pvIndex = 0; pvIndex < rootMove.pv.size(); ++pvIndex) {
          if (pvIndex)
            json << ",";
          json << "\"" << json_escape(UCI::move(pos, rootMove.pv[pvIndex])) << "\"";
        }
        json << "]";
        json << "}";
      }
      ...
      std::cout.rdbuf(previousCout);
      return std::string("{\"ok\":false,\"error\":\"") + json_escape(error.what()) + "\"}";
    }
    catch (...) {
      std::cout.rdbuf(previousCout);
      return "{\"ok\":false,\"error\":\"unknown analyze error\"}";
    }
  }
}
...
EMSCRIPTEN_BINDINGS(ffish_js) {
  ...
  function("analyze", &ffish::analyze);
